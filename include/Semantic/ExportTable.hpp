...
public:
    ...
    [[nodiscard]] bool hasExport(const std::string& moduleName, const std::string& symbolName) const;

    // returns all the export associated with a module
    [[nodiscard]] std::unordered_set<std::string> getExports(const std::string& moduleName) const;

    // Checks if a module exists in the export table
    [[nodiscard]] bool moduleExists(const std::string& moduleName) const;
